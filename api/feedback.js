...
export default async function handler(req, res) {
  ...
  const { question, answer, presetPrompt, framework, questionType, selfIntent, jobPosting, previousAnswer, charLimit } = req.body || {};
  if (!question || !answer) {
    return res.status(400).json({ error: '자소서 문항과 답변이 필요합니다.' });
  }

  const isOverLimit = charLimit && answer.length > charLimit;
...
${isOverLimit ? `\n[글자수 초과 경고]\n이 문항의 글자수 제한은 ${charLimit}자인데, 학생 답변은 ${answer.length}자로 ${answer.length - charLimit}자 초과했습니다. charLimitNote 필드에 "OO자 초과했습니다. [어느 문장/표현]을 줄이면 됩니다"처럼, 실제 답변에서 구체적으로 어느 부분을 줄이면 되는지 짚어서 1~2문장으로 안내하세요. polishedText는 반드시 이 글자수 제한(${charLimit}자) 안에 들어오도록 작성하세요.\n` : ''}
[구체성 체크]
...
[AI 작성 흔적 안내 — 문구 단위로 콕 집어주기]
학생 답변 안에서 실제로 존재하는 문구 중, AI가 자소서를 대신 써줄 때 흔히 쓰는 상투적 표현(예: "열정을 다해", "최선을 다하겠습니다", "책임감을 가지고", "성장하는 사람이 되겠습니다" 같이 근거 없이 감정·다짐만 나열하는 문구)이 있다면 최대 2개까지 골라서, 그 문구를 답변에서 그대로 정확히 인용하고, 왜 상투적인지와 본인만의 경험으로 어떻게 바꾸면 좋을지 짧게 제안하세요. 답변에 실제로 없는 문구를 지어내지 마세요. 해당하는 문구가 없으면 빈 배열로 두세요.
...
반드시 아래 JSON 형식으로만 응답하세요. 인사말, 설명, 코드블록 표시(\`\`\`) 등 다른 텍스트는 절대 포함하지 마세요. 각 문장형 필드는 간결하게 작성하세요 (missingElements는 최대 3개, strengths·improvements·interviewerInference·followUpQuestions의 각 항목은 1~2문장 이내). intentGapComment는 학생 자가진단이 제공된 경우에만 채우고, 없으면 null로 두세요. jobMatch는 채용공고 내용이 제공된 경우에만 채우고, 없으면 null로 두세요. revisionComment는 이전 버전이 제공된 경우에만 채우고, 없으면 null로 두세요. charLimitNote는 글자수 초과인 경우에만 채우고, 없으면 null로 두세요.
{
  ...
  "aiTracePhrases": [{"phrase": "답변 속 실제 문구를 그대로 인용", "note": "왜 상투적인지 + 본인 경험으로 바꾸는 방향 제안, 1문장"}] 또는 해당 없으면 빈 배열,
  ...
  "charLimitNote": "글자수 초과 시 어느 부분을 줄이면 되는지 안내, 초과 없으면 null",
